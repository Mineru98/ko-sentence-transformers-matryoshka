...
nli_train_dataloader = datasets.NoDuplicatesDataLoader(
    nli_train_samples, batch_size=args.nli_batch_size
)
base_loss = CoSENTLoss(model=model)
nli_train_loss = MatryoshkaLoss(
    model=model,
    loss=base_loss,
...
sts_train_dataloader = DataLoader(
    sts_train_samples, shuffle=True, batch_size=args.sts_batch_size
)
...
warmup_steps = math.ceil(
    steps_per_epoch * args.num_epochs * 0.1
)  # 10% of train data for warm-up
logging.info("Warmup-steps: {}".format(warmup_steps))

# Train the model
train_objectives = [
    (nli_train_dataloader, nli_train_loss),
    (sts_train_dataloader, sts_train_loss),
]
model.fit(
